...
function nextStep() {
  const steps = document.querySelectorAll('.step-by-step .step:not(.visible)');
  if (steps.length > 0) {
    steps.item(0).classList.add('visible');
  } else {
    next()
  }
};
...
<div class="browser" id="browser">
  ...
  showFile(index) {
    ...
    xhr.open('GET', `/demoit/sourceCode/${folder}/${files[index]}?style=vs${segment}`);
    xhr.send();
  }
});
...
<div class="browser" id="browser">
    <div class="browser-navigation-bar">
        ...
        <input id="browser-url" value="${src}" />
...
`;
    ...
    window.addEventListener('message', (message) => {
      if (/^navigate:.*/.test(message.data)) {
        this.shadowRoot.getElementById("site").src = message.data.replace('navigate:', '');
        this.shadowRoot.getElementById("browser-url").value = this.shadowRoot.getElementById("site").src;
        this.refresh()
      }
    });
  ...
  fetchUntilSuccess(url, callback) {
    ...
    xhr.open('HEAD', `/demoit/ping?url=${url}`);
    xhr.send();
  }
});
...
<div class="browser">
    ...
    xhr.open('GET', `/demoit/tree/${folder}`);
    xhr.send();
  }
...
window.addEventListener('load', function () {
  const speakerNotes = document.querySelector("aside.notes");
  if (!speakerNotes) return;
  Array.from(speakerNotes.childNodes).forEach(node => {
    if (node.nodeType === Node.TEXT_NODE) {
      let text = node.textContent;
      text = text.trim();
      if (text != '') {
        console.log("%c" + text, window.getComputedStyle(node.parentNode).cssText);
      }
    } else {
